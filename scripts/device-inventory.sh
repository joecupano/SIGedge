...
echo "== Reading this =="
echo "An 'unpinned' radiod mission or a Kismet source with no serial/index"
echo "constraint will grab whichever matching device it finds first -- fine"
echo "with exactly one unit of that device type attached, a real collision"
echo "risk with more than one. Pin by serial (radiod: RTLSDR_SERIAL= to"
echo "scripts/cfg_ka9q-radio; Kismet: rtl433-sn-<serial> in"
echo "kismet_site.conf) whenever two always-on consumers need the same"
echo "device type. See README.md's device-ownership section and this"
echo "script's own header comment."
